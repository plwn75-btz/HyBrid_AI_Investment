...
def start_ai_ranking():
    ...
    global _ai_results_store, _ai_start_time
    ...
    with _ai_lock:
        # Auto-unlock stale runs after 60 seconds
        if _ai_progress['running'] and (time.time() - _ai_start_time > 60):
        ...
        if _ai_progress['running']:
            return jsonify({
                ...
                'message': 'AI Selection is already in progress. Connecting to active analysis pipeline...',
                'stage': _ai_progress.get('stage', 1)
            }), 200
...
@screening_bp.route('/api/ai_rank/cancel', methods=['POST'])
def cancel_ai_ranking():
    """Cancel background AI Stock Selection task."""
    global _ai_results_store
    with _ai_lock:
        _ai_progress['running'] = False
        _ai_progress['done'] = True
        _ai_progress['message'] = 'Analysis Cancelled by User.'
        _ai_results_store = {'status': 'cancelled', 'message': 'AI Selection task was cancelled.'}
