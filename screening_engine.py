...
THAI_HOLIDAYS_VARIABLE = {
    # 2024
    (2024, 2, 26): "Makha Bucha (Substitution)",
    (2024, 5, 22): "Visakha Bucha",
    (2024, 7, 20): "Asarnha Bucha",
    # 2025
    (2025, 2, 12): "Makha Bucha",
    (2025, 5, 11): "Visakha Bucha",
    (2025, 7, 10): "Asarnha Bucha",
    # 2026
    (2026, 3, 3): "Makha Bucha",
    (2026, 5, 31): "Visakha Bucha",
    (2026, 7, 29): "Asarnha Bucha"
}


def get_thai_holiday_name(date_obj):
    """Returns holiday name string if date_obj is a Thai Public Holiday, else None."""
    var_key = (date_obj.year, date_obj.month, date_obj.day)
    if var_key in THAI_HOLIDAYS_VARIABLE:
        return THAI_HOLIDAYS_VARIABLE[var_key]

    fixed_key = (date_obj.month, date_obj.day)
    if fixed_key in THAI_HOLIDAYS_FIXED:
        return THAI_HOLIDAYS_FIXED[fixed_key]
...
def is_trading_day(date_obj):
    """Check if date is an active SET trading day (not a weekend and not a Thai Public Holiday)."""
    if date_obj.weekday() >= 5:
        return False
    if get_thai_holiday_name(date_obj) is not None:
        return False
    return True


def adjust_to_last_friday(date_obj):
    """Adjust date to the last valid SET trading day if it falls on a weekend or holiday."""
    curr = date_obj
...
def get_default_date():
    """Calculate default screening date (latest valid trading day)."""
    today = date.today()
    target = today - timedelta(days=1)
    resolved_str, _ = resolve_date(target.strftime("%Y-%m-%d"))
    return datetime.strptime(resolved_str, "%Y-%m-%d").date()


def resolve_date(date_str):
    """
    Parse date string and adjust for weekends & Thailand Public Holidays.
    Recursively steps back day-by-day until a valid active SET trading day is found.
    Returns (resolved_date_str, message).
    """
    try:
        input_date = datetime.strptime(str(date_str).strip(), "%Y-%m-%d").date()
    except Exception:
        input_date = date.today() - timedelta(days=1)

    curr_date = input_date
    reasons = []

    while not is_trading_day(curr_date):
        if curr_date.weekday() >= 5:
            day_name = curr_date.strftime("%A")
            reasons.append(f"{curr_date.strftime('%Y-%m-%d')} ({day_name})")
        else:
            h_name = get_thai_holiday_name(curr_date) or "Public Holiday"
            reasons.append(f"{curr_date.strftime('%Y-%m-%d')} ({h_name})")
        curr_date -= timedelta(days=1)

    resolved_str = curr_date.strftime("%Y-%m-%d")
    if curr_date == input_date:
        msg = f"{resolved_str} is an active SET trading day."
    else:
        msg = f"{input_date.strftime('%Y-%m-%d')} is a non-trading day ({', '.join(reasons)}). Adjusted to latest trading day {resolved_str}."

    return resolved_str, msg
