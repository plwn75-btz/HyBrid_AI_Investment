...
def compute_stock_valuation(yf_raw):
    ...
    peg = safe_float(info_dict.get("pegRatio") or yf_raw.get("peg") or yf_raw.get("peg_ratio"))
    de_ratio = safe_float(info_dict.get("debtToEquity") or yf_raw.get("de") or yf_raw.get("debt_to_equity"))
    div_yield = safe_float(info_dict.get("dividendYield") or yf_raw.get("dividend_yield") or yf_raw.get("div_yield"))
    if div_yield > 100.0:
        div_yield = div_yield / 100.0
    elif 0 < div_yield < 1.0:
        div_yield = div_yield * 100.0
    net_margin = safe_float(info_dict.get("profitMargins") or yf_raw.get("npm") or yf_raw.get("net_margin"))
    if 0 < net_margin < 1.0:
...
def run_ai_stock_selection(weights=None, date_str=None, rsi=30, stoch=70, min_criteria=4, set50_only=False, index_filter="all", process_all=True, progress_callback=None):
    ...
    summary_info = screen_results.get('summary', {})
    resolved_date = summary_info.get('date', date_str)
    date_message = summary_info.get('date_message', '')
    results_dict = screen_results.get('results', {})
    
    min_c = max(1, min(6, int(min_criteria)))

    def collect_candidates(threshold):
        collected = []
        for c in range(6, threshold - 1, -1):
            for s_item in results_dict.get(str(c), []):
                s_copy = dict(s_item)
                s_copy['criteria_passed'] = c
                collected.append(s_copy)
        return collected

    matched_stocks = collect_candidates(min_c)
    ...
    if index_filter == "set50":
        filtered = [s for s in matched_stocks if is_set50(s.get('symbol') or s.get('Symbol', ''))]
        if not filtered:
            # Fallback 1: Auto-relax down to 3 criteria for SET50
            matched_stocks_low = collect_candidates(3)
            filtered = [s for s in matched_stocks_low if is_set50(s.get('symbol') or s.get('Symbol', ''))]
            if filtered:
                auto_relaxed = True
                auto_relaxed_msg = f"⚠️ Warning: 0 stocks met {min_c} technical criteria for SET50 on {date_str}. Auto-Relaxation Applied (Displaying candidates matching 3 criteria)."
        if not filtered:
            # Fallback 2: Try 2 criteria
            matched_stocks_low = collect_candidates(2)
            filtered = [s for s in matched_stocks_low if is_set50(s.get('symbol') or s.get('Symbol', ''))]
            if filtered:
                auto_relaxed = True
                auto_relaxed_msg = f"⚠️ Warning: 0 stocks met {min_c} criteria for SET50 on {date_str}. Auto-Relaxed to Min 2 Criteria."
        matched_stocks = filtered
    elif index_filter == "set100":
        filtered = [s for s in matched_stocks if is_set100(s.get('symbol') or s.get('Symbol', ''))]
        if not filtered:
            # Fallback 1: Auto-relax down to 3 criteria for SET100
            matched_stocks_low = collect_candidates(3)
            filtered = [s for s in matched_stocks_low if is_set100(s.get('symbol') or s.get('Symbol', ''))]
            if filtered:
                auto_relaxed = True
                auto_relaxed_msg = f"⚠️ Warning: 0 stocks met {min_c} technical criteria for SET100 on {date_str}. Auto-Relaxation Applied (Displaying candidates matching 3 criteria)."
        if not filtered:
            matched_stocks_low = collect_candidates(2)
            filtered = [s for s in matched_stocks_low if is_set100(s.get('symbol') or s.get('Symbol', ''))]
            if filtered:
                auto_relaxed = True
                auto_relaxed_msg = f"⚠️ Warning: 0 stocks met {min_c} criteria for SET100 on {date_str}. Auto-Relaxed to Min 2 Criteria."
        matched_stocks = filtered

    if not matched_stocks:
        # Fallback 3: If still empty at min_c (e.g. 4), try collecting down to 1 criteria for full SET
        matched_stocks = collect_candidates(1)

    if not matched_stocks:
        idx_msg = f" in {index_filter.upper()} Index" if index_filter != "all" else ""
        return {
            "status": "error",
            "message": f"No stocks matched technical screening criteria on {date_str}{idx_msg}.",
            "rankings": [],
            "generated_at": datetime.now().isoformat()
        }

    # Priority sort: 6 criteria -> 5 criteria -> 4 criteria -> 3 criteria
    ...
    notify(1, 100, 100, f"Stage 1 Complete: Found {len(matched_stocks)} stocks. Analyzing {total_candidates} candidates.")
    ...
    for idx, row in enumerate(candidates, 1):
        ...
        candidate_dataset.append({
            'symbol': sym,
            'price': price,
            'sector': yf_raw.get('sector') or yf_raw.get('info', {}).get('sector', 'N/A'),
            'fair_value': fair_val,
    ...
    return {
        ...
        "auto_relaxed_msg": auto_relaxed_msg,
